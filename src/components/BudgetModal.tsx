...
export const BudgetModal: React.FC<BudgetModalProps> = ({ isOpen, onClose, propertyTitle }) => {
  const [budget, setBudget] = useState('');
  const [purpose, setPurpose] = useState('');
  const [rentalIncome, setRentalIncome] = useState('');

  const handleSubmit = () => {
    const message = `Hi, I'm interested in ${propertyTitle === 'Custom Property Search' ? 'finding properties' : `exploring more properties like "${propertyTitle}"`}. Here are my requirements:
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative bg-white rounded-t-3xl w-full max-w-lg mx-4 mb-0 transform transition-transform duration-300 ease-out animate-in slide-in-from-bottom">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h3 className="text-base font-semibold text-gray-900">Tell us your requirements</h3>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {['Below 50 Lakh', '50-99 Lakh', '1-1.5 Cr', '1.5-2.5 Cr', '2.5-5 Cr', '5 Cr+'].map((range) => (
                <button
                  key={range}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {['Investment with Rental', 'Self Use'].map((option) => (
                <button
                  key={option}
